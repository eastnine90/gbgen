...
// checks if the FeatureRule type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FeatureRule{}
...
type FeatureRule struct {
	...
	FeatureSafeRolloutRule *FeatureSafeRolloutRule
}
...
func (dst *FeatureRule) UnmarshalJSON(data []byte) error {
	...
	// check if the discriminator value is 'experiment'
	if jsonDict["type"] == "experiment" {
	...
	// check if the discriminator value is 'experiment-ref'
	if jsonDict["type"] == "experiment-ref" {
	...
	// check if the discriminator value is 'force'
	if jsonDict["type"] == "force" {
	...
	// check if the discriminator value is 'rollout'
	if jsonDict["type"] == "rollout" {
	...
	// check if the discriminator value is 'safe-rollout'
	if jsonDict["type"] == "safe-rollout" {
		// try to unmarshal JSON data into FeatureSafeRolloutRule
		err = json.Unmarshal(data, &dst.FeatureSafeRolloutRule);
		if err == nil {
			jsonFeatureSafeRolloutRule, _ := json.Marshal(dst.FeatureSafeRolloutRule)
			if string(jsonFeatureSafeRolloutRule) == "{}" { // empty struct
				dst.FeatureSafeRolloutRule = nil
			} else {
				return nil // data stored in dst.FeatureSafeRolloutRule, return on the first match
			}
		} else {
			dst.FeatureSafeRolloutRule = nil
		}
...
func (src FeatureRule) MarshalJSON() ([]byte, error) {
	...
	if src.FeatureSafeRolloutRule != nil {
		return json.Marshal(&src.FeatureSafeRolloutRule)
	}

	return nil, nil // no data in anyOf schemas
}

func (src FeatureRule) ToMap() (map[string]interface{}, error) {
	if src.FeatureExperimentRefRule != nil {
		return src.FeatureExperimentRefRule.ToMap()
	}

	if src.FeatureExperimentRule != nil {
		return src.FeatureExperimentRule.ToMap()
	}

	if src.FeatureForceRule != nil {
		return src.FeatureForceRule.ToMap()
	}

	if src.FeatureRolloutRule != nil {
		return src.FeatureRolloutRule.ToMap()
	}

	if src.FeatureSafeRolloutRule != nil {
		return src.FeatureSafeRolloutRule.ToMap()
	}
